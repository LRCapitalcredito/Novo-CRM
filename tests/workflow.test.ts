...
import { newBank } from "../src/directory";
...
const placement = (store: ReturnType<typeof createPreviewStore>, op: Operation, id="workflow-placement") => { if (!store.directory.state().records.some(r => r.id === "workflow-bank")) store.directory.save({...newBank(),id:"workflow-bank",name:"Instituição fictícia"},null); return store.records.save(recordInput("placement",op.id,{ bankId:"workflow-bank",institution:"Instituição fictícia",managerId:"",manager:"Gerente fictício",status:"Em análise",product:"",requestedCents:null,approvedCents:null,notes:"",nextAction:"Consultar andamento",dueDate:"",active:true },id),null); };
